...
var UserAgent = "Golang oyster-iot/client package"
...
func SetAppName(userApp string) error {
	UserAgent = fmt.Sprintf(
		"Golang/oyster-iot (%s; %s; %s) %s", runtime.GOOS, runtime.GOARCH, userApp, runtime.Version())
	return nil
}
...
func ResponseError(resp *http.Response) (err error) {
	...
	if resp.StatusCode > 299 {
		if resp.ContentLength != 0 {
			...
			e.Err = fmt.Sprintf("Response Status Code:%d", e.Code)
		}
	}
	return e
}
